...
TESTS = [
    ...
    Test("TestEmptyOutputArgsFail", "--output_format=", "--output_file=",
         ["Want form --{option_name}={option_value}"], "", [""]),
    Test("TestEmptyOutputFormatOk", "", "--output_file=results",
         ["Generated results"], "results", ["Running benchmarks in"]),
...
def test_output_flags():
    ...
    for t in TESTS:
        if t.test_output_file_arg and not t.want_file_contents:
            printf(
                "{t.name} is an invalid test. If a file is being generated, it should have expected output"
            )
            continue
        test_call = [
            binary_under_test,
